...
const MapRender = ({ lat, long, zoom }) => {
	...
	let mapContainer = mapBuild.mapHolder;
	let pointers = mapBuild.pointerHolder;
	...
	useEffect(
		() => {
			mapContainer = L.map('map', mapOptions);
		},
		[]
	);
	...
	useEffect(
		() => {
			if (pointers) pointers.setLatLng(libri);
			else {
				pointers = libri.map((single) =>
					L.marker({ lat: single.lat, lng: single.lon }, { icon: customIcon })
						.addTo(mapContainer)
						.bindPopup(single.title)
				);
			}
		},
		[ libri, customIcon ]
	...
	return <div id="map" style={{ width: `${width}` }} />;
};
